...
def UploadAct(request):
	...
	try:
		...
		#user validation
		users = json.loads(requests.get(user_service_ip+'/api/v1/users').text)
		...
		c = models.act.objects.create(actId=int(data['actId']), username=data["username"], timestamp=formatted_timestamp, caption=data['caption'], categoryName=categoryName, imgB64=imgB64)
